...
import { isFunction, isPropertyKey } from "./Asserts.js";
...
_a = Actions, _b = onEnter, _c = onExit, _d = onCanExit, _e = onUpdate;
/**
...
_f = To, _g = From, _h = Condition, _j = Event;
/**
...
export class FiniteStateMachine {
    constructor(args = {}) {
        ...
        Object.defineProperty(this, "globalEventTransitions", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new ReverseMap()
        ...
        if (t[Event]) {
            if (t[From]) {
                if (!this.eventTransitions.has(t[Event]))
                    ...
                    this.eventTransitions.get(t[Event]).add(t);
            }
            else {
                if (!this.globalEventTransitions.has(t[Event]))
                    this.eventTransitions.set(t[Event], new Set([t]));
                else
                    this.globalEventTransitions.get(t[Event]).add(t);
            }
        }
        else {
    ...
    /** @internal */
    [(_k = ProvidedUpdate, _l = Actions, _m = onEnter, _o = onExit, _p = onCanExit, ExitCurrentState)]() {
        if (this.currentState instanceof FiniteStateMachine)
    ...
    [GetMatchingTransition]() {
        for (const transition of this.globalTransitions)
            if (transition[To] !== this.states.getKey(this.currentState) && isFunction(transition[Condition]) && transition[Condition]())
                return transition;
